...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        self.params[f"W{1}"] = np.random.normal(loc=0.0, scale=weight_scale,
                                                size=(num_filters, C, filter_size, filter_size))
        self.params[f"b{1}"] = np.zeros((num_filters,))

        self.params[f"W{2}"] = np.random.normal(loc=0.0, scale=weight_scale, size=(num_filters * H *  W // 4, hidden_dim)) # divide by four due to pooling
    ...
    def loss(self, X, y=None):
        ...
        dA2, dW3, db3 = affine_backward(dout, cache3)
        dA1, dW2, db2 = affine_relu_backward(dA2, cache2)
        dX, dW1, db1 = conv_relu_pool_backward(dA1, cache1)

        grads[f"W1"] = dW1 + self.reg * W1
        grads[f"W2"] = dW2 + self.reg * W2
        grads[f"W3"] = dW3 + self.reg * W3
